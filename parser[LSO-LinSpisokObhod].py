#!/usr/bin/env python3
"""
VPN Config Collector v14.2 (добавлены заголовки в файлы)
...
- Источники: RKPchannel, EtoNeYaProject, rtwo2/FastNodes
- В начало all.txt, LTE.txt, WiFi.txt добавлены заголовки
"""
...
def extract_sni_domain(config: str) -> Optional[str]:
    """Извлекает значение параметра sni= (домен или IP)."""
    protocol = None
...
def rename_config(config: str) -> str:
    """Переименовывает конфиг: удаляет старый комментарий, добавляет #sni | тип (без пробела перед #)"""
    protocol = None
    for p in PROTOCOL_PATTERNS:
        if config.startswith(p + "://"):
            protocol = p
...
def classify_config(config: str, whitelist: Set[str]) -> str:
    """Классификация по sni: lte если sni в whitelist, иначе wifi."""
...
def update_readme(stats: Dict, sources_count: int):
    ...
    readme_content = (
        ...
        "## 🔄 Автообновление\n\n"
        f"Скрипт `{SCRIPT_NAME}` запускается каждые 30 минут через GitHub Actions.\n"
...
def save_configs(all_configs_set: Set[str]):
    ...
    # Текущее время для подстановки
    update_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # Добавляем маркировку ко всем конфигам
    tagged_set = {f"{cfg} {GLOBAL_TAG}" for cfg in all_configs_set}
    sorted_configs = sorted(tagged_set)

    # Шаблоны заголовков
    header_all = f"""#profile-title: #LSO-#LinSpisokObhod
#profile-update-interval: 1
#support-url: https://t.me/VPNLin_channel
#announce: LinSpisokObhod подписка all.txt рекомендуем для использованием мобильным интернетом и вайфаем . Время обновления: {update_time} #LinSpisokObhod
...
#profile-update-interval: 1
#support-url: https://t.me/VPNLin_channel
#announce: LinSpisokObhod подписка LTE.txt рекомендуем для использованием мобильного интернета. Время обновления: {update_time} #LinSpisokObhod
#subscription-userinfo: upload=0; download=0; total=0; expire=0
...
#profile-update-interval: 1
#support-url: https://t.me/VPNLin_channel
#announce: LinSpisokObhod подписка WIFI.txt рекомендуем для использованием вайфаем . Время обновления: {update_time} #LinSpisokObhod
#subscription-userinfo: upload=0; download=0; total=0; expire=0

"""

    # Сохраняем all.txt с заголовком
    all_txt_path = os.path.join(CONFIG_DIR, "all.txt")
    with open(all_txt_path, 'w', encoding='utf-8') as f:
        f.write(header_all)
        f.write("\n".join(sorted_configs) + "\n")
    logger.info(f"💾 Общий файл {all_txt_path} ({len(sorted_configs)} уникальных конфигов)")
    ...
    # Сохраняем LTE.txt с заголовком
    ...
    # Сохраняем WiFi.txt с заголовком
    ...
    # Подсчёт по протоколам для статистики
...
def main():
    ...
    print("=" * 60)
    print(f"🚀 {SCRIPT_NAME} v14.2 (заголовки в файлах, запуск каждые 30 минут)")
    print("=" * 60)
