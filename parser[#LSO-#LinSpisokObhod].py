...
# === ФУНКЦИЯ ДЛЯ РАСПОЗНАВАНИЯ И ДЕКОДИРОВАНИЯ BASE64 ===
def decode_base64_if_needed(content: str) -> str:
    ...
    try:
        ...
        if len(decoded_str) > 0 and any(c.isprintable() for c in decoded_str):
            # Если в декодированной строке есть символы '://' (признак URL конфига) или она содержит конфиги
            if '://' in decoded_str or 'vless://' in decoded_str or 'vmess://' in decoded_str or 'trojan://' in decoded_str or 'hysteria2://' in decoded_str or 'ss://' in decoded_str:
                return decoded_str
