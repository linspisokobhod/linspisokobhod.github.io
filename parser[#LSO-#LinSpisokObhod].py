...
async def collect_configs_async(contents: Dict[str, Optional[str]]) -> Set[str]:
    ...
    if raw_configs:
        ...
        if unique_endpoints:
            logger.info(f"🌐 Уникальных эндпоинтов для проверки: {len(unique_endpoints)}")
            sem = asyncio.Semaphore(20)
            async def check_endpoint(host, port):
                async with sem:
                    return host, port, await check_tcp_port(host, port)
...
async def main_async():
    ...
    print("=" * 60)
    print("🚀 LinSpisokObhod (TCP-тестирование 30 сек, обновлённые источники)")
    print("=" * 60)
