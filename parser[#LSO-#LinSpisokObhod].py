...
def decode_base64_if_needed(content: str) -> str:
    ...
    try:
        ...
        if '://' in decoded_str or 'vless://' in decoded_str or 'vmess://' in decoded_str:
            return decoded_str
...
async def collect_configs_async(contents: Dict[str, Optional[str]]) -> Set[str]:
    ...
    # ---- TCP-ТЕСТИРОВАНИЕ (ТАЙМАУТ 30 СЕКУНД) ----
    if raw_configs:
        ...
        if unique_endpoints:
            ...
            # Отфильтровываем конфиги по рабочим эндпоинтам
            filtered_configs = []
            for cfg in raw_configs:
                host = extract_host_from_config(cfg)
                if host:
...
def generate_extra_lines(total: int, protocols: Dict[str, int], update_time: str, sub_type: str = "ALL") -> List[str]:
    ...
    lines.append(f"ss://YWVzLTI1Ni1nY206c3M@ss.about.lso:443#Подписка #LSO© SS: {protocols.get('ss', 0)}")
...
# === СОХРАНЕНИЕ БЕЗ ЗАГОЛОВКОВ (С ФИЛЬТРАЦИЕЙ SS) ===
...
"""
    ...
    update_readme(len(tagged_set), len(lte_list), len(wifi_list), protocol_counts_all)
    logger.info("✅ Готово.")
...
async def main_async():
    ...
    print("=" * 60)
    print("🚀 LinSpisokObhod (SS исключены)")
    print("=" * 60)
